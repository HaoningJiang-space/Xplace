...
def get_option():
    ...
    parser.add_argument('--oracle_topfrac', type=float, default=0.0, help='MECHANISM_AUTOPSY #3 (cross-design fairness): if >0, keep top FRACTION of ALL nets by criticality (constant fraction across designs; supersedes fixed --oracle_topk which is a different fraction per design)')
    parser.add_argument('--oracle_pin_weight_mode', type=str, default='broadcast', help="MECHANISM_AUTOPSY #2 (decisive mechanism test): 'broadcast'=net weight on ALL pins (current; high-fanout=more total pull=geometry bias); 'fanout_norm'=divide by net degree (fanout-neutral). +15% surviving fanout_norm => WHICH-nets (timing-like); collapsing => fanout/geometry")
    ...
    parser.add_argument('--timing_route_weight', type=float, default=0.0, help='TERM-2 prototype (MATH.md/PROTOTYPE.md): scale the per-node congestion (route) force by (1+alpha*node_criticality) so timing-critical cells are pushed out of congestion (route-aware DENSITY lever, NOT the HPWL pull). Needs --use_route_force True + a criticality source (oracle_timing_file or timing_opt). 0 = off (uniform route force).')
    parser.add_argument('--timing_force_frac', type=float, default=0.0, help='AUTO-CALIBRATE the timing force to a design-invariant FRACTION of the WL gradient norm (R25 fix): if >0, rescale the timing gradient each iter so ||timing_grad|| = frac*||wl_grad||, preserving per-pin criticality RANKING but auto-setting the global scale. Frozen rule that transfers across designs (vs a fixed --oracle_timing_scale that over/under-pulls). 0 = off (use raw scale).')
    parser.add_argument('--detour_timing_weight', type=float, default=0.0, help='Level-A differentiable route-aware timing (IMPLICIT_DIFF_TIMING.md §9): weight of an additive term ∂[Σ w_n·HPWL_n·(1+alpha·rho_n)]/∂x, where w_n=net_criticality and rho_n=differentiable pin-density congestion at the net centroid. Tests whether a DIFFERENTIABLE routed-length (detour) signal beats the frozen route-blind HPWL force, one-directional (no routing var). Needs a criticality source (timing_opt / oracle). 0 = off.')
    parser.add_argument('--detour_alpha', type=float, default=2.0, help='detour strength for --detour_timing_weight: routed-length ~= est_length*(1+alpha*congestion).')
    parser.add_argument('--detour_arc', type=str2bool, default=False, help='Level-A granularity (CRITICALITY_GRANULARITY.md): False=net-level (Σ w_n·HPWL_n·(1+α·ρ_centroid)); True=ARC-level per-critical-pin (Σ_v κ_v·‖pos(v)−c_net(v)‖·(1+α·ρ_v)) — acts on the critical pin with its OWN corridor congestion (per-sink detour), avoiding net aggregation false-negatives. codex arch-review: arc-level is the correct unit.')
