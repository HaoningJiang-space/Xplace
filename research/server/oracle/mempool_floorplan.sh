#!/bin/bash
# 2nd HIGH-DIVERGENCE design candidate: produce mempool_group 2_floorplan.odb via ORFS (synth+floorplan+RTLMP
...
cd $F
echo "MEMPOOL_FLOORPLAN_START $(date +%H:%M:%S)"
...
make YOSYS_CMD=$YOS OPENROAD_EXE=$ORD DESIGN_CONFIG=designs/nangate45/mempool_group/config.mk floorplan
echo "MEMPOOL_FLOORPLAN_DONE rc=$? $(date +%H:%M:%S)"
ls -la $F/results/nangate45/mempool/base/2_floorplan.odb 2>/dev/null || echo "NO 2_floorplan.odb produced"
