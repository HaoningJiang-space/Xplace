#!/bin/bash
# 2nd HIGH-DIVERGENCE design candidate: produce bp_quad 2_floorplan.odb via ORFS (synth+floorplan+RTLMP
...
cd $F
echo "BPQUAD_FLOORPLAN_START $(date +%H:%M:%S)"
...
make YOSYS_CMD=$YOS OPENROAD_EXE=$ORD DESIGN_CONFIG=designs/nangate45/bp_quad/config.mk floorplan
echo "BPQUAD_FLOORPLAN_DONE rc=$? $(date +%H:%M:%S)"
ls -la $F/results/nangate45/bp_quad/base/2_floorplan.odb 2>/dev/null || echo "NO 2_floorplan.odb produced"
