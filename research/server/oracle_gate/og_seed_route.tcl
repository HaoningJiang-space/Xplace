# og_seed_route.tcl : re-route the SAME placement (pre-route db w/ global routes)
# with a given detailed-route seed, write SPEF, and dump per-arc routed RC.
# Single openroad process. Env:
#   OG_LIBS OG_LEFS OG_PREROUTE_DB(5_2_fillcell.odb) OG_SDC OG_OUT OG_TAG
#   OG_SEED  OG_MINLAYER OG_MAXLAYER OG_NUMCORES
...
set args [list -output_drc $OUT/${TAG}_drc.rpt -or_seed $::env(OG_SEED) -verbose 1 -droute_end_iter 5]
if {[info exists ::env(OG_MINLAYER)] && $::env(OG_MINLAYER) ne ""} { lappend args -bottom_routing_layer $::env(OG_MINLAYER) }
if {[info exists ::env(OG_MAXLAYER)] && $::env(OG_MAXLAYER) ne ""} { lappend args -top_routing_layer $::env(OG_MAXLAYER) }
puts "SEED_ROUTE seed=$::env(OG_SEED) args=$args"
detailed_route {*}$args

# extract real parasitics from this seed's routing
set rcx $::env(OG_RCX_RULES)
if {[file exists $rcx]} {
