...
- **ariane133/NanGate45 full ORFS flow** (synth done → macro-place running → place→CTS→route→SPEF). THE macro-design go/no-go: Gate-A beyond-RUDY + seed-causality on a congested macro design. ← most important pending result.
- (Deferred) ASAP7 macro gate awaits either an SRAM-LEF pin-access fix or a different ASAP7 macro design.
