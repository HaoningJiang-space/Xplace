...
## NEXT (unblocked 2026-06-17)
1. Calibrate `wire_resistance/capacitance_per_micron` for NanGate45 (or read from setRC) so GPUTimer timing is sane.
2. Real `--timing_opt` (fair estimated baseline) vs the routed-RC-corrected `--timing_opt` (the thesis mechanism) → post-route TNS. Force/seed-matched, WNS/Fmax metrics.
